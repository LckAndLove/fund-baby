...
- 持仓收益：可录入持仓份额和成本，计算当前估值、日收益、持有收益和收益率；也可将基金设为不计入持仓收益，仅保留行情展示。
- 桌面小组件：Windows 下支持无边框透明悬浮窗、置顶、拖动、托盘菜单和快捷键。
- F2 快捷键：按 `F2` 在主面板和日收益小条之间切换；恢复主面板时会自动刷新一次。
- 托盘设置：支持开机自启动、透明度调节、配置导入导出、显示/隐藏和退出。
...
- 托盘图标左键：显示主面板并刷新。
- 托盘菜单：开机自启动、透明度、导入配置、导出配置、显示、隐藏到托盘、退出。
- 面板内刷新按钮：立即拉取最新行情。
- 编辑模式：维护基金列表、持仓份额、成本和是否计入收益；基金超过 10 个时，持仓表格会在面板内滚动。

导入/导出的配置包含基金列表、自选、分组、折叠状态、刷新频率、视图模式、持仓和待处理交易记录。
